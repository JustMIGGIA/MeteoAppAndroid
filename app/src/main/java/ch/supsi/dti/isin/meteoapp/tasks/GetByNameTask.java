...
public class GetByNameTask extends AsyncTask<String, Void, Location> {

    public GetByNameTask(){}

    @Override
    protected Location doInBackground(String... strings) {
        Location loc = null;
        try {
            URL url = new URL("https://api.openweathermap.org/data/2.5/weather?q="+ strings[0] +"&units=metric&lang=it&appid=" + Constants.KEY);
